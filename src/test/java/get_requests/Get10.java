...
public class Get10 extends HerokuAppBaseUrl {
   /*
    ...
    Map<String,String> bookingData= obje.getBookingDates("2018-01-01","2019-01-01");
    Map<String,Object> expectedData = obje.getPayLoad("John","Smith",111,true,bookingData,"Breakfast");
        // Send the request and get the response
        Response response = given(spec).when().get("{first}/{second}");
        response.prettyPrint();
        // Do assertion
        ...
        assertEquals(bookingData.get("checkout"), ((Map)actualData.get("bookingdates")).get("checkout"));
        // 2. YOL
        JsonPath json = response.jsonPath();
